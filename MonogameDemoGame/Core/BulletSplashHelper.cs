...
namespace MonogameDemoGame.Core
{
    ...
    {
        public static IEnumerable<Vector2> Spawn(Random random, IEnumerable<CollisionSplashStruct> collisionSplashes,
            int NumberOfCollisionSplashParticlesToCreate, int MaximumSqrtOfAngleToThrowCollisionSplashParticleInDegrees)
        {
            ...
            foreach (var splash in collisionSplashes)
            {
                ...
                {
                    int randomNumber = RandomHelper.NextRandomNumberBetweenPositiveAndNegative(random, MaximumSqrtOfAngleToThrowCollisionSplashParticleInDegrees);
